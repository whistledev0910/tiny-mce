...
(function () {
    ...
    const global$2 = tinymce.util.Tools.resolve('tinymce.PluginManager');
    ...
    const prismjs = function (global, module, exports) {
        ...
        function matchGrammar(text, tokenList, grammar, startNode, startPos, rematch) {
          for (const token in grammar) {
            ...
            for (let j = 0; j < patterns.length; ++j) {
              ...
              for (let currentNode = startNode.next, pos = startPos; currentNode !== tokenList.tail; pos += currentNode.value.length, currentNode = currentNode.next) {
                ...
                for (let i = 0; i < tokens.length; i++) {
                  ...
                  if (typeof token === 'string' || token.content && typeof token.content === 'string') {
                    ...
                    if (index > -1) {
                      ...
                      const middle = new Prism.Token(language, Prism.tokenize(t, env.grammar), `language-${  language}`, t);
      ...
      (function (Prism) {
        ...
        const modName = /\b(?!<keyword>)\w+(?:\s*\.\s*\w+)*\b/.source.replace(/<keyword>/g, function () {
      ...
      (function (Prism) {
        ...
        const generic = nested(/<(?:[^<>;=+\-*/%&|^]|<<self>>)*>/.source, 2);
        const nestedRound = nested(/\((?:[^()]|<<self>>)*\)/.source, 2);
        const name = /@?\b[A-Za-z_]\w*\b/.source;
        const genericName = replace(/<<0>>(?:\s*<<1>>)?/.source, [
          name,
          generic
        ]);
        ...
        const attr = replace(/<<0>>(?:\s*\(<<1>>*\))?/.source, [
          identifier,
          roundExpression
        ]);
        Prism.languages.insertBefore('csharp', 'class-name', {
    ...
    const global = tinymce.util.Tools.resolve('tinymce.util.Tools');
